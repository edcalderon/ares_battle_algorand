...
export const getAppsFromAddressByKey = async (Address: string, key: string) => {
    const accountInfo = await algorand.client.algod.accountInformation(Address).do();
    const allApps = accountInfo['createdApps']

    const filtered = (allApps || []).filter(app => {
        const globalState = app.params.globalState;
        return globalState ? decodeGlobalState(globalState as any).decodedStates.some(state => state.key === key) : false;
    });
    return filtered;
}
